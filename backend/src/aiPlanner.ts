...
async function providerPlan(prompt: string, dashboard: DashboardDocument): Promise<DashboardPlan | null> {
  ...
  const isGemini = model.toLowerCase().startsWith("gemini-") || baseUrl.includes("generativelanguage.googleapis.com");
  const tokenLimit = isGemini ? { max_tokens: 16384 } : { max_completion_tokens: 1800 };
  const response = await fetch(`${baseUrl.replace(/\/$/, "")}/chat/completions`, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, messages: [{ role: "system", content: `You are AvhiSafe's dashboard planner. Return only safe per-user UI operations. Never change vault security, private keys, transaction signing, or other users. Supported widgets: ${widgetCatalog.map((widget) => widget.type).join(", ")}. Current dashboard: ${JSON.stringify({ name: dashboard.name, theme: dashboard.theme, currency: dashboard.currency, density: dashboard.density, widgets: dashboard.widgets })}` }, { role: "user", content: prompt }], response_format: { type: "json_schema", json_schema: { name: "avhisafe_dashboard_plan", strict: true, schema: responseSchema } }, ...tokenLimit }) });
