...
export async function planDashboardUi(prompt: string, dashboard: DashboardDocument, currentUi?: UISpec): Promise<UIPlan> {
  ...
  const system = [
    ...
    "Every component must be one of widget, text, metric, card, badge, list, or divider. Use widgets for live registered data, metrics for compact summaries, cards for explanations and warnings, lists for short checklists, badges for status, and dividers for sections. IDs must be lowercase kebab/snake identifiers and unique.",
