...
export async function planDashboardUi(prompt: string, dashboard: DashboardDocument, currentUi?: UISpec): Promise<UIPlan> {
  ...
  const system = [
    ...
    "Return JSON only matching the supplied schema. Never return JSX, TSX, JavaScript, HTML, CSS, SQL, URLs, shell commands, or executable code.",
    ...
    "Return exactly this JSON envelope: { intent: \"customize_ui\", explanation: \"short explanation\", warnings: [], requiresApproval: true, ui: { version: 1, title: \"...\", description: \"...\", accentPreset: \"cyan\", layout: \"grid\", columns: 2, components: [] } }.",
    ...
    "Every component must be one of widget, text, metric, or card. IDs must be lowercase kebab/snake identifiers and unique.",
