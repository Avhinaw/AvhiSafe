...
export default function ConnectedWallets() {
  ...
  return <section className="rounded-2xl border border-primary/10 bg-primary/[0.03] p-6"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-primary/50">Third-party wallets</p><h2 className="mt-1 text-2xl font-black">Connect and view</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-primary/60">Connect MetaMask or another EVM wallet, or connect Phantom and another Solana wallet. AvhiSafe requests only the public address and never signs or sends transactions.</p></div><WalletCards className="size-7 text-primary/40" /></div><div className="mt-5 flex flex-wrap gap-3"><Button onClick={() => connect("ethereum")} disabled={busy !== null} className="gap-2"><Link2 className="size-4" />{busy === "ethereum" ? "Connecting…" : "Connect MetaMask / EVM"}</Button><Button onClick={() => connect("solana")} disabled={busy !== null} variant="outline" className="gap-2">{busy === "solana" ? "Connecting…" : "Connect Solana wallet"}</Button></div><div className="mt-4 flex flex-wrap gap-3 text-xs text-primary/50"><span>{evmDetected ? "EVM provider detected" : "No EVM provider detected"}</span><span>{solanaDetected ? "Solana provider detected" : "No Solana provider detected"}</span></div>{wallets.length > 0 && <div className="mt-5 grid gap-3 md:grid-cols-2">{wallets.map((wallet) => <div key={wallet.id} className="flex items-center justify-between rounded-xl bg-background/70 p-4"><div><p className="text-xs font-bold uppercase tracking-wider text-primary/50">{wallet.chain === "ethereum" ? "EVM" : "Solana"} · {wallet.provider}</p><p className="mt-1 font-mono text-sm">{shortAddress(wallet.address)}</p><p className="mt-1 text-xs text-emerald-600">Connected for read-only viewing</p></div><Button variant="ghost" size="icon" onClick={() => disconnectWallet(wallet)} aria-label={`Disconnect ${wallet.chain} wallet`}><LogOut className="size-4" /></Button></div>)}</div>}</section>;
}
