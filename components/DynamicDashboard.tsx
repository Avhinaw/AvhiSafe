...
function GeneratedComponent({ component }: { component: UIComponent }) {
  if (component.type === "widget") return <div className="rounded-2xl border border-primary/10 bg-background/60 p-3"><div className="mb-3 px-2 text-xs font-bold uppercase tracking-wider text-primary/45">{component.title || component.widgetType.replaceAll("-", " ")}</div><RegisteredWidget widgetType={component.widgetType} /></div>;
  ...
  if (component.type === "metric") return <div className="rounded-2xl border border-primary/10 bg-background/60 p-5"><p className="text-xs font-bold uppercase tracking-wider text-primary/50">{component.label}</p><p className="mt-3 text-sm text-primary/65">Live {component.source.replaceAll("-", " ")} data</p><p className="mt-1 text-xs text-primary/45">Format: {component.format}</p></div>;
  ...
  return <div className={`rounded-2xl border p-5 ${toneClass}`}><p className="font-semibold">{component.title || "AI-generated card"}</p>{(component.text || component.body) && <p className="mt-2 text-sm text-primary/65">{component.text || component.body}</p>}</div>;
}

function GeneratedUi({ document }: { document: UIDocument }) {
  const spec = document.spec;
  const components = useMemo(() => spec.components, [spec.components]);
  return <div className={`rounded-3xl border p-4 sm:p-6 ${accentClasses[spec.accentPreset]}`} data-ui-version={document.version} data-ui-source={document.source}><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary/50">AI-generated personal interface · v{document.version}</p><h3 className="mt-2 text-3xl font-black tracking-tight">{spec.title}</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-primary/60">{spec.description}</p></div><div className={spec.layout === "grid" ? "grid gap-4" : "flex flex-col gap-4"} style={spec.layout === "grid" ? { gridTemplateColumns: `repeat(${spec.columns}, minmax(0, 1fr))` } : undefined}>{components.map((component) => <GeneratedComponent key={component.id} component={component} />)}</div></div>;
}
...
export default function DynamicDashboard() {
  ...
  const load = async () => { setLoading(true); setBackendError(null); try { const data = await loadWorkspace(); setDashboard(data.dashboard); setUiDocument(data.uiDocument); } catch (error) { setBackendError(error instanceof Error ? error.message : "The AI workspace backend is unavailable."); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
