  ...
  return (
    ...
    >
      <div className="mb-6">
        ...
        <h3 className="mt-2 text-3xl font-black tracking-tight">{spec.title || "AvhiSafe workspace"}</h3>
      ...
      <div
        className={spec.layout === "grid" ? "grid gap-4" : "flex flex-col gap-4"}
        style={spec.layout === "grid" ? { gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` } : undefined}
      >
...
export default function DynamicDashboard() {
  ...
  if (loading) return (
    <section className="mt-12 rounded-3xl border border-primary/10 p-8 text-center">
      <Loader2 className="mx-auto size-8 animate-spin text-primary/40" />
      <p className="mt-4 text-sm text-primary/60">Loading your AI-generated personal workspace…</p>
    </section>
  );

  if (!dashboard || !uiDocument) return (
    <section className="mt-12 rounded-3xl border border-amber-500/30 bg-amber-500/[0.05] p-8">
  ...
  return (
    <section className="mt-12 flex flex-col gap-6 rounded-3xl border border-primary/10 bg-background p-4 pt-12 text-foreground transition-colors duration-200 sm:p-6 sm:pt-12">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-primary/50">Universal AI workspace</p>
          <h2 className="mt-2 flex items-center gap-3 text-4xl font-black tracking-tight"><LayoutDashboard className="size-8" />{dashboard.name}</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-primary/60">This interface is generated by AI, saved in MongoDB, and isolated to this dashboard and user.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-700">AI UI · synced</span>
