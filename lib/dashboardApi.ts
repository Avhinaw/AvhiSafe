...
export type DashboardPlan = { intent: "customize_ui" | "unsupported"; explanation: string; warnings: string[]; requiresApproval: boolean; ui: UISpec | null; source: "ai"; model?: string };

const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || "https://avhisafe-backend.onrender.com";
...
const FRIENDLY_ERRORS: Record<string, string> = {
  "Failed to fetch": "Unable to connect to the backend. Please check your internet connection.",
  "Load failed": "Unable to connect to the backend. Please check your internet connection.",
...
async function request<T>(path: string, options: RequestInit = {}, timeoutMs = 45000): Promise<T> {
  ...
  try {
    const response = await fetch(`${backendUrl}${path}`, {
      ...
      headers: { "Content-Type": "application/json", "x-user-id": getDashboardUserId(), ...(options.headers || {}) },
    });
    if (!response.ok) {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error("Request timed out. The AI may be processing — please try again.");
    }
...
export function planDashboard(id: string, prompt: string) { return request<{ plan: DashboardPlan; request: unknown }>("/api/v1/ai/plan", { method: "POST", body: JSON.stringify({ dashboardId: id, prompt }) }, 60000); }
